...
public class LuongDAL
{
    ...
    // Get detailed salary data for an employee
    public DataRow GetLuongChiTiet(string maNV)
    {
        using (SqlConnection conn = new SqlConnection(connectionString))
        {
            conn.Open();
            string query = @"
            SELECT l.TongNgayLam, l.TongNgayNghi, l.SoGioTre, l.TroCap, l.TamUng
            FROM NhanVien nv
    ...
    public List<LuongDTO> GetDanhSachLuong()
    {
        ...
        using (SqlConnection conn = new SqlConnection(connectionString))
        {
            conn.Open();
            string query = @"
            SELECT L.MaNV, NV.TenNV, L.MaLuong, L.NgayXetLuong, 
                   L.TongNgayLam, L.TongNgaynghi, L.TroCap, L.TamUng
            FROM Luong L
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                while (reader.Read())
                {
                    danhSach.Add(new LuongDTO
                    {
                        ...
                        TongNgayLam = Convert.ToInt32(reader["TongNgayLam"]),  // Đảm bảo tên trường là đúng
                        TongNgaynghi = Convert.ToInt32(reader["TongNgaynghi"]),  // Đảm bảo tên trường là đúng
